...
const UNIVERSITY_ID = "afd4182c-30fd-4cb6-973a-015f06d2cdb4";
...
async function main() {
    ...
    await prisma.university
        .deleteMany({ where: { id: { not: UNIVERSITY_ID } } })
        .catch(() => {});
    ...
    if (!uni) {
        throw new Error(
            `University ${UNIVERSITY_ID} not found. Please create it first.`
        );
    }
    ...
    await prisma.user
        .deleteMany({ where: { role: { not: "ADMIN" } } })
        .catch(() => {});
    ...
    for (let i = 1; i <= 50; i++) {
        const user = await prisma.user.create({
            data: {
                ...
                password:
                    "$2a$10$4y6ZQqK8R3d5C2w9iKeGEO1w9P6h0o8x2hTjo0p5Xx9xXbT2m3w2S", // bcrypt for 'password'
    ...
    const notices = [
        {
            ...
            content: "Orientation will be held next week.",
            type: "GENERAL",
            priority: "LOW",
            targetAudience: "ALL",
        },
        {
            ...
            content: "Please check the exam portal.",
            type: "EXAM",
            priority: "HIGH",
